...
static int realtek_gpio_probe(struct platform_device *pdev)
{
	...
	if (np) {
		if (!of_property_read_bool(np, "base")) {
			ctrl->gc.base = -1;
		} else {
			u8 base = 0;
	...
	if (np && of_property_read_bool(np, "interrupt-controller")) {
		...
		irq_parent = (unsigned int)irq;
		ctrl->gc.irq.chip = &realtek_gpio_irqchip;
		ctrl->gc.irq.handler = handle_edge_irq;
		ctrl->gc.irq.default_type = IRQ_TYPE_NONE;
		ctrl->gc.irq.parent_handler = realtek_gpio_irq_handler;
		ctrl->gc.irq.parents = devm_kmemdup(&pdev->dev, &irq_parent,
						    sizeof(irq_parent),
						    GFP_KERNEL);
		if (!ctrl->gc.irq.parents)
			return -ENOMEM;
		ctrl->gc.irq.num_parents = 1;
	}
	...
	if (np && of_property_read_bool(np, "interrupt-controller"))
		dev_info(&pdev->dev, "Realtek GPIO controller at IRQ %d\n", irq);
	else
		dev_info(&pdev->dev, "Realtek GPIO controller\n");
